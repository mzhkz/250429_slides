...
\title{\textsc{SFC-RG General Lecture} \\ Threat Clarification and Correctness Proofs in Protocol Design}
\author{
...
\begin{itemize}
    ...
    \item What conditions must be satisfied for the protocol to be considered secure
...
\begin{proofblock}[Adversary]
An adversary refers to an attacker, typically with malicious intent, who undertakes actions against a system or protocol\\
An adversary can control entities participating in the protocol, modifying or monitoring their operations.
\end{proofblock}

\vspace{0.5em}
\begin{proofblock}[Threat Model]
A \textcolor{red}{threat model} formalizes assumptions about adversary capabilities and behaviors.
\end{proofblock}
...
\begin{figure}
    ...
    \caption{Relationship between adversaries and entities. Adversaries can introduce corrupted entities into the protocol by intercepting messages from honest entities or directly controlling entities.\footnote{Note that entities do not become corrupted directly!}}
...
\begin{itemize}
    ...
    \item \textbf{\textcolor{red}{Adaptive}}: Adversaries can dynamically select entities to corrupt during protocol execution.
\end{itemize}
...
In security protocol research, \textcolor{red}{clarifying assumptions} is extremely important.
...
\begin{itemize}
    \item Enables accurate understanding of \textcolor{red}{protocol correctness}
...
\begin{itemize}
    ...
    \item \textbf{Mobility}: Mobile - Adversaries can dynamically corrupt different replicas in each phase of the protocol
\end{itemize}
\end{proofblock}
\end{frame}
...
Assumption: Suppose two different values $v$ and $v'$ were committed with the same sequence number $s$.

\begin{lemma}
For each value to be committed, PREPARE messages from at least $2f+1$ replicas are required. (Out of $3f+1$ replicas, at most $f$ can be corrupted)
\end{lemma}

\begin{itemize}
    \item \small For value $v$ to be committed, PREPARE messages from at least $2f+1$ replicas in set $\mathcal{P}$ are required
    \item \small Similarly, for value $v'$ to be committed, PREPARE messages from at least $2f+1$ replicas in set $\mathcal{Q}$ are required
    \item \small Out of a total of $n \leq 3f+1$ replicas, at most $f$ can be malicious
    \item \small $| \mathcal{P} \cap \mathcal{Q} | \geq 2f+1 + 2f+1 - n \geq 4f+2 - (3f+1) = f+1$
    \item \small This means at least $f+1$ \textcolor{red}{honest replicas} sent PREPARE messages for both values $v$ and $v'$
    \item \small However, the maximum number of corrupted replicas is $f$, and honest replicas do not send PREPARE messages for multiple values with the same sequence number (by protocol definition)
    \item \small This is a contradiction, so the assumption is incorrect
\end{itemize}
\end{frame}
...
\begin{frame}{Correctness Claims in UC}
  ...
  \begin{proofblock}[Definition of UC Correctness]
   If the environment $\mathcal{Z}$ tries to find differences between interactions in the real world and the ideal world but cannot distinguish between them, then $\pi$ can be said to correctly realize the ideal functionality $\mathcal{F}$.\footnote{This is called "$\pi$ UC-realizes $\mathcal{F}$"}
...
\begin{frame}{Benefits of Using UC for Correctness Proofs}
  \begin{itemize}
    ...
    \item \textbf{Composition Theorem. } If protocol $\pi$ UC-realizes ideal functionality $\mathcal{F}$ and protocol $\rho$ UC-realizes ideal functionality $\mathcal{G}$ using $\mathcal{F}$, then the protocol $\rho^{\mathcal{F} \rightarrow \pi}$ obtained by replacing $\mathcal{F}$ with $\pi$ also UC-realizes $\mathcal{G}$.
  \end{itemize}
