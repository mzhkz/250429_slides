...
PBFTは、\textcolor{red}{Byzantine障害を許容する状態マシンレプリケーション}プロトコルである。

\vspace{0.5em}

% レプリカの定義を追加
\textbf{レプリカ}：分散システム内で同一の状態を維持し、同一の操作を実行する複製ノード。

\vspace{0.5em}

前提：$n \geq 3f+1$、ここで$f$は敵対者の数、$n$はレプリカの総数

\vspace{0.5em}

このとき、正直なレプリカは、敵対者の影響を受けずに、「データをLinearlizable」に複製できる。
...
\begin{enumerate}
    ...
    \item \textbf{\textcolor{red}{Commit}}：少なくとも$2f+1$のPREPAREメッセージを受信したレプリカは$\langle \text{COMMIT}, s, r \rangle$メッセージを送信し、実行を確約
...
PBFTの脅威モデル：
\begin{itemize}
    \item \textbf{腐敗}：\textcolor{red}{Byzantine} - レプリカは異なるメッセージを異なるレプリカに送信するなど、任意に振る舞うことができる
    \item \textbf{適応性}：Adaptive - 敵対者はプロトコルの各フェーズで異なるレプリカを動的に腐敗させられる
    \item \textbf{計算}：PPT - 多項式時間の計算に制限される（暗号署名が偽造不可能と仮定）
    \item \textbf{可視性}：Private channels - 正直なレプリカ間の通信内容は秘密であり、内部状態も敵対者からは見えない
\end{itemize}
\end{frame}
...
SafetyとLivenessを両方満たす場合、PBFTはセキュアであると言える。

\vspace{0.5em}

\textbf{なぜ$n \geq 3f + 1$のときSafetyが満たされるのか？}\\

\vspace{0.5em}

\begin{tcolorbox}[colframe=black, colback=white, boxrule=1pt, boxsep=5pt]
\textbf{Question}: PBFTがセキュアであることをどのように証明するか？説明してみよう。
\end{tcolorbox}
\end{frame}

\begin{frame}{プロトコルセキュリティの証明}
