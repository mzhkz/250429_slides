...
\footnote{プロトコルは英語や日本語など共通のルールで会話する例として説明されることが多いが、これは相互作用の規則を表しているためである。一方で「日本語アルゴリズム」という表現が一般的でないのは、アルゴリズムが相互作用ではなく計算手順を表すためである。}
一般的なプロトコルの例：HTTP、SMTP、TLS/SSL、SSH、OAuth、Paxos、Raft

\end{frame}

\begin{frame}{安全なプロトコルの設計}
...
\begin{itemize}
    ...
    \item \textbf{そのようなエンティティが存在するときに、プロコトルが安全であるかどうかをどのように判断するか}
\end{itemize}
...
\begin{itemize}
    \item \small 値$v$がコミットされるには少なくとも$2f+1$個のレプリカ$\mathcal{P}$からのPREPAREメッセージが必要
    \item \small 同様に、値$v'$がコミットされるには少なくとも$2f+1$個のレプリカ$\mathcal{Q}$からのPREPAREメッセージが必要
